...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tabButtons = {
        'feed-data-tab-btn': { icon: 'fa-database', title: 'Feed Data', desc: '' },
        'calculation-tab-btn': { icon: 'fa-calculator', title: 'Calculation', desc: 'Hydraulic projection, recovery calculations, salt passage, and pressure estimates.' },
        'scaling-tab-btn': { icon: 'fa-triangle-exclamation', title: 'Scaling & Fouling Analysis', desc: 'Solubility indices (LSI, SDSI), supersaturation calculations, and antiscalant dosage guides.' },
        'process-tab-btn': { icon: 'fa-wand-magic-sparkles', title: 'Process Recommendation', desc: 'System design flows, chemical dosing loops, and system arrangement setups.' },
        'decline-tab-btn': { icon: 'fa-chart-line', title: 'Performance Decline', desc: 'Membrane compaction, flux decline projections, fouling rates, and temperature permeability corrections over time.' },
        'report-tab-btn': { icon: 'fa-file-pdf', title: 'Report', desc: 'Exporting comprehensive project analysis summary sheets to PDF and Excel.' }
    };

    const feedDashboard = document.getElementById('feed-dashboard-view');
    const declineDashboard = document.getElementById('decline-panel-view');
    const placeholderView = document.getElementById('tab-placeholder-view');
    const scalingDashboard = document.getElementById('scaling-panel-view');
    const processDashboard = document.getElementById('process-dashboard-view');
    const calcPanel = document.getElementById('calculation-panel-view');
    ...
    Object.keys(tabButtons).forEach(id => {
        ...
        if (btn) {
            btn.addEventListener('click', () => {
                ...
                // Store active tab ID to persist across refreshes
                localStorage.setItem('activeTabId', id);

                const reportDashboard = document.getElementById('report-panel-view');
                if (id === 'feed-data-tab-btn') {
                    // Show feed data dashboard
                    feedDashboard.style.display = 'grid';
                    declineDashboard.style.display = 'none';
                    if (reportDashboard) reportDashboard.style.display = 'none';
                    if (scalingDashboard) scalingDashboard.style.display = 'none';
                    if (processDashboard) processDashboard.style.display = 'none';
                    if (calcPanel) calcPanel.style.display = 'none';
                    placeholderView.style.display = 'none';
                } else if (id === 'decline-tab-btn') {
                    // Show performance decline dashboard
                    feedDashboard.style.display = 'none';
                    declineDashboard.style.display = 'grid';
                    if (reportDashboard) reportDashboard.style.display = 'none';
                    if (scalingDashboard) scalingDashboard.style.display = 'none';
                    if (processDashboard) processDashboard.style.display = 'none';
                    if (calcPanel) calcPanel.style.display = 'none';
                    ...
                    // Sync all parameters from Feed Data
                    syncAllParametersFromFeed();
                } else if (id === 'scaling-tab-btn') {
                    // Show scaling dashboard
                    feedDashboard.style.display = 'none';
                    declineDashboard.style.display = 'none';
                    if (reportDashboard) reportDashboard.style.display = 'none';
                    if (scalingDashboard) scalingDashboard.style.display = 'grid';
                    if (processDashboard) processDashboard.style.display = 'none';
                    if (calcPanel) calcPanel.style.display = 'none';
                    placeholderView.style.display = 'none';
                    ...
                    // Fetch membrane catalogs
                    fetch('http://localhost:8000/api/membranes')
                        .then(res => res.json())
                        .then(data => {
                            if(data.ro_membranes) {
                                const roSel = document.getElementById('calc-ro-membrane');
                                roSel.innerHTML = data.ro_membranes.map(m => `<option value="${m.id}">${m.name} (${m.type})</option>`).join('');
                            }
                            if(data.uf_modules) {
                                const ufSel = document.getElementById('calc-uf-module');
                                ufSel.innerHTML = data.uf_modules.map(m => `<option value="${m.id}">${m.name}</option>`).join('');
                            }
                        })
                        .catch(err => console.error('Failed to load membrane DB', err));

                } else if (id === 'process-tab-btn') {
                    // Show process recommendation dashboard
                    feedDashboard.style.display = 'none';
                    declineDashboard.style.display = 'none';
                    if (reportDashboard) reportDashboard.style.display = 'none';
                    if (scalingDashboard) scalingDashboard.style.display = 'none';
                    if (processDashboard) processDashboard.style.display = 'grid';
                    if (calcPanel) calcPanel.style.display = 'none';
                    placeholderView.style.display = 'none';
                } else if (id === 'membrane-tab-btn') {
                    // Show membrane recommendation dashboard
                    feedDashboard.style.display = 'none';
                    declineDashboard.style.display = 'none';
                    if (reportDashboard) reportDashboard.style.display = 'none';
                    if (scalingDashboard) scalingDashboard.style.display = 'none';
                    if (processDashboard) processDashboard.style.display = 'none';
                    if (calcPanel) calcPanel.style.display = 'none';
                    if (membraneDashboard) {
                        membraneDashboard.style.display = 'grid';
                        
                        // Sync settings from Feed Data
                        const targetTdsInput = document.getElementById('target-tds');
                        
                        
                        // Run recommendation
                        fetchMembraneRecommendation();
                    }
                    placeholderView.style.display = 'none';
                    ...
                    if (calcPanel) calcPanel.style.display = 'none';
                    if (reportDashboard) {
                        reportDashboard.style.display = 'grid';
                    ...
                    const info = tabButtons[id];
                    placeholderIcon.className = `fa-solid ${info.icon} placeholder-icon`;
                    placeholderTitle.textContent = info.title;
                    placeholderDesc.textContent = info.desc + ' This module is under development and will be integrated in the next projection phase.';
                }
            });
        }
    });
    ...
    const savedTabId = localStorage.getItem('activeTabId') || 'feed-data-tab-btn';
    const savedBtn = document.getElementById(savedTabId);
    if (savedBtn) {
        savedBtn.click();
    } else {
        document.getElementById('feed-data-tab-btn').click();
    }
...
async function runPhreeqcCalculation() {
    ...
    try {
        ...
        if (window.siChart) {
            ...
            // Dynamic coloring (vibrant rose red if > 0, else sky blue)
            window.siChart.data.datasets[0].backgroundColor = window.siChart.data.datasets[0].data.map(val => {
                ...
                return val > 0 ? 'rgba(244, 63, 94, 0.85)' : 'rgba(14, 165, 233, 0.85)';
            });
            window.siChart.data.datasets[0].borderColor = window.siChart.data.datasets[0].data.map(val => {
                if (val === null) return 'rgba(0,0,0,0)';
                return val > 0 ? 'rgba(244, 63, 94, 1)' : 'rgba(14, 165, 233, 1)';
            });
        ...
        <div class="carousel-outer" style="display: flex; flex-direction: column; gap: 0.6rem; width: 100%; position: relative; flex-grow: 1; height: 100%; justify-content: space-between;">
            <div style="display: flex; align-items: center; gap: 0.6rem; width: 100%; flex-grow: 1;">
                ...
                <div class="carousel-slide-container" style="flex-grow: 1; min-width: 0; transition: transform 0.3s ease, opacity 0.3s ease; opacity: 1; height: 100%; display: flex; flex-direction: column;" id="active-candidate-slide">
                    <div class="candidate-card" style="border: 1px solid ${isBest ? 'rgba(59, 130, 246, 0.5)' : 'var(--card-border)'}; background: ${isBest ? 'linear-gradient(135deg, rgba(30, 41, 59, 0.4), rgba(15, 23, 42, 0.6))' : 'rgba(30, 41, 59, 0.2)'}; border-radius: 6px; padding: 0.8rem; display: flex; flex-direction: column; gap: 0.6rem; flex-grow: 1; height: 100%; justify-content: space-between;">
                        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                            <div style="display: flex; align-items: center; gap: 0.8rem;">
                                <div style="font-size: 1.3rem; font-weight: 800; color: #f8fafc; font-family: 'Inter', sans-serif;">
                                    ${rec.model}
                                    ${isBest ? ' <span style="font-size:0.68rem; vertical-align:middle; background:rgba(59, 130, 246, 0.2); color:#60a5fa; border:1px solid rgba(59,130,246,0.4); padding:0.1rem 0.4rem; border-radius:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-left:0.4rem;">RECOMMENDED</span>' : ''}
                                </div>
                        ...
                        <div style="font-size: 0.82rem; color: var(--text-secondary); line-height: 1.35; border-left: 2px solid ${scoreColor}; padding-left: 0.7rem; margin: 0;">
                            Permeate TDS: <b>${rec.calculated_metrics.permeate_tds} mg/L</b> | Energy: <b>${rec.calculated_metrics.specific_energy} kWh/m³</b><br>
                            Feed Pressure: <b>${rec.calculated_metrics.feed_pressure_bar} bar</b> | Max CP (\u03B2): <b>${rec.max_beta ? rec.max_beta.toFixed(2) : '1.00'}</b>
                        </div>
                        
                        <!-- Details Panel -->
                        <div style="display: flex; flex-direction: column; gap: 0.7rem; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 0.7rem; margin-top: 0.3rem;">
                            <!-- DQ Banner if disqualified -->
                            ${isDQ ? `
                            <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.3); color: #f87171; padding: 0.6rem; border-radius: 4px; font-size: 0.82rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;">
                                <i class="fa-solid fa-triangle-exclamation"></i> Disqualified: ${rec.disqualification_reason}
                            </div>
                            ` : ''}

                            <!-- Operating Envelope Limits Checklist -->
                            <div style="background: rgba(0,0,0,0.1); border-radius: 4px; padding: 0.4rem 0.7rem; border: 1px solid rgba(255,255,255,0.03);">
                                <h4 style="margin-top:0; margin-bottom:0.4rem; font-size:0.75rem; font-weight:700; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.05em;">Criteria Scores</h4>
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.4rem; font-size:0.78rem;">
                                    <span style="color:#60a5fa"><i class="fa-solid fa-droplet"></i> Rejection: ${rec.criteria_scores.rejection || 0}/30</span>
                                    <span style="color:#60a5fa"><i class="fa-solid fa-bolt"></i> Energy: ${rec.criteria_scores.energy || 0}/30</span>
                                    <span style="color:#60a5fa"><i class="fa-solid fa-water"></i> Hydraulic: ${rec.criteria_scores.hydraulic || 0}/20</span>
                                    <span style="color:#60a5fa"><i class="fa-solid fa-gauge-high"></i> Envelope: ${rec.criteria_scores.envelope || 0}/20</span>
                                </div>
                            </div>

                            <!-- Evaluation Notes -->
                            <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                                <h4 style="margin: 0; font-size:0.75rem; font-weight:700; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.05em;">Engineering Notes</h4>
                                <ul style="padding-left: 1rem; margin: 0; font-size: 0.78rem; color: var(--text-primary); line-height:1.45; display:flex; flex-direction:column; gap:0.15rem;">
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        title: { display: true, text: 'Saturation Index (log10)', color: '#94a3b8', font: { family: 'Inter', size: 13 } }
                    },
                    x: {
                        grid: { display: false },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Toggle UF module dropdown based on train selection
    ...
    if (trainSelect) {
        trainSelect.addEventListener('change', (e) => {
            ...
            if (e.target.value.includes('UF')) {
                ufGroup.style.display = 'block';
            } else {
                ufGroup.style.display = 'none';
            }
        });
    }
    ...
    if (calcBtn) {
        calcBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const vesselsStr = document.getElementById('calc-vessels-array').value;
                const vessels = vesselsStr.split(',').map(s => parseInt(s.trim()) || 1);
                ...
                const payload = {
                    ...
                    feed_water: feedData,
                    target_flow_m3h: parseFloat(document.getElementById('flow').value) || 100,
                    target_recovery_pct: parseFloat(document.getElementById('calc-target-recovery').value) || 75,
                    ro_membrane: document.getElementById('calc-ro-membrane').value,
                    uf_module: document.getElementById('calc-tech-train').value.includes('UF') ? document.getElementById('calc-uf-module').value : null,
                    stages: parseInt(document.getElementById('calc-stages').value) || 2,
                    vessels_per_stage: vessels,
                    elements_per_vessel: parseInt(document.getElementById('calc-elements-pv').value) || 6,
                    recycle_enabled: document.getElementById('calc-recycle-enable') ? document.getElementById('calc-recycle-enable').checked : false,
                    recycle_ratio: document.getElementById('calc-recycle-ratio') ? parseFloat(document.getElementById('calc-recycle-ratio').value) / 100.0 : 0.0,
                    economic_params: {
                        electricity_tariff: parseFloat(document.getElementById('eco-tariff').value) || 7.50,
                        membrane_cost: parseFloat(document.getElementById('eco-mem-cost').value) || 26880,
                        vessel_cost: parseFloat(document.getElementById('eco-ves-cost').value) || 48000,
                        pump_cost_kw: parseFloat(document.getElementById('eco-pump-cost').value) || 96000,
                        ic_factor: (parseFloat(document.getElementById('eco-ic-factor').value) || 15) / 100,
                        contingency_factor: (parseFloat(document.getElementById('eco-cont-factor').value) || 10) / 100,
                        membrane_lifetime: parseFloat(document.getElementById('eco-mem-life').value) || 5,
                        plant_availability: (parseFloat(document.getElementById('eco-avail').value) || 90) / 100,
                        discount_rate: 0.10,
                        project_life: 20
                    }
                };

                const res = await fetch('http://localhost:8000/api/calculate-system', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
                ...
                // Render UF results
                if (data.uf_results) {
                    results.querySelector('#calc-uf-results-card').style.display = 'block';
                    results.querySelector('#calc-uf-modules').innerText = data.uf_results.overview.total_modules;
                    results.querySelector('#calc-uf-flux').innerText = data.uf_results.operating_conditions.filtration_flux_lmh.toFixed(1);
                    results.querySelector('#calc-uf-rec').innerText = data.uf_results.overview.recovery_pct.toFixed(1);
                    results.querySelector('#calc-uf-tmp').innerText = data.uf_results.overview.tmp_design_bar.toFixed(2);
                } else {
                    results.querySelector('#calc-uf-results-card').style.display = 'none';
                }
                ...
                if (data.economics) {
                    ...
                    // CAPEX
                    document.getElementById('eco-capex-mem').innerText = formatINR(eco.capex.membranes_inr);
                    document.getElementById('eco-capex-ves').innerText = formatINR(eco.capex.vessels_inr);
                    document.getElementById('eco-capex-hp').innerText = formatINR(eco.capex.hp_pump_inr);
                    document.getElementById('eco-capex-bp').innerText = formatINR(eco.capex.booster_pump_inr);
                    document.getElementById('eco-capex-sub').innerText = formatINR(eco.capex.equip_subtotal_inr);
                    document.getElementById('eco-capex-ic').innerText = formatINR(eco.capex.ic_inr);
                    document.getElementById('eco-capex-cont').innerText = formatINR(eco.capex.contingency_inr);
                    document.getElementById('eco-capex-total').innerText = formatINR(eco.capex.total_capex_inr);
                    
                    // OPEX
                    document.getElementById('eco-opex-energy').innerText = formatINR(eco.opex.energy_cost_pa_inr);
                    document.getElementById('eco-opex-mem').innerText = formatINR(eco.opex.membrane_repl_pa_inr);
                    document.getElementById('eco-opex-total').innerText = formatINR(eco.opex.total_opex_pa_inr);
                    
                    // Unit Cost
                    document.getElementById('eco-metrics-kl').innerText = new Intl.NumberFormat('en-IN').format(eco.metrics.annual_production_kl);
                    document.getElementById('eco-metrics-tac').innerText = formatINR(eco.metrics.total_annual_cost_inr);
                    document.getElementById('eco-metrics-cost-kl').innerText = '₹' + eco.metrics.cost_per_kl_inr.toFixed(2);
                }
                
                // Render RO results
                if (data.ro_results) {
                    const roSum = data.ro_results.summary;
                    results.querySelector('#calc-sys-rec').innerText = (roSum.total_recovery * 100).toFixed(1);
                    results.querySelector('#calc-sys-perm-flow').innerText = roSum.perm_flow.toFixed(1);
                    results.querySelector('#calc-sys-press').innerText = roSum.feed_pressure_bar.toFixed(1);
                    results.querySelector('#calc-sys-tds').innerText = roSum.perm_tds.toFixed(1);
                    results.querySelector('#calc-sys-sec').innerText = roSum.sec_kwh_m3.toFixed(2);
                    ...
                    // Render Stage-wise Table
                    const tbodyStage = results.querySelector('#calc-stage-tbody');
                    if (tbodyStage && data.ro_results.stages) {
                        tbodyStage.innerHTML = data.ro_results.stages.map(s => `
                            <tr class="stage-summary-row" data-stage="${s.stage}" style="cursor: pointer;" onclick="toggleStageElements(${s.stage}, this)">
                                <td style="text-align: left; font-weight: 600;"><i class="fa-solid fa-chevron-right" id="stage-chevron-${s.stage}" style="margin-right: 0.5rem; color: var(--primary-color); transition: transform 0.2s; transform: rotate(0deg);"></i> Stage ${s.stage}</td>
                    ...
                    if (data.ro_results.booster_pumps && data.ro_results.booster_pumps.length > 0) {
                        ...
                        if (requiredPumps.length > 0) {
                            bpCard.style.display = 'block';
                            tbodyBP.innerHTML = requiredPumps.map(bp => `
                                <tr>
                    ...
                    // Render Element-wise Table
                    const tbodyHyd = results.querySelector('#calc-hyd-tbody');
                    const vesselsPerStage = roSum.vessels_per_stage || [];
                    let hydHtml = '';
                    
                    for (let stageIdx = 0; stageIdx < vesselsPerStage.length; stageIdx++) {
                        const stageElements = data.ro_results.elements.filter(e => e.stage === (stageIdx + 1));
                        
                        hydHtml += stageElements.map(e => `
                            <tr class="stage-row-${e.stage}" style="display: none;">
                                <td>S${e.stage}-E${e.position}</td>
                    ...
                    tbodyHyd.innerHTML = hydHtml;
                    
                    // Render Ion Rejection Table
                    const tbodyIon = results.querySelector('#calc-ion-tbody');
                    let ionsHtml = '';
                    const ionMap = { 'Ca': 'calcium', 'Mg': 'magnesium', 'Na': 'sodium', 'K': 'potassium', 'Ba': 'barium', 'Sr': 'strontium', 'Cl': 'chloride', 'SO4': 'sulfate', 'HCO3': 'bicarbonate', 'NO3': 'nitrate', 'F': 'fluoride', 'SiO2': 'silica', 'B': 'boron', 'PO4': 'phosphate', 'NH4': 'ammonium' };
                    for (const [ion, feedC] of Object.entries(roSum.conc_ions)) { // just to get keys
                        ...
                        const p = roSum.perm_ions[ion] || 0;
                        const c = roSum.conc_ions[ion] || 0;
                        let rej = 0;
                        if (f > 0) rej = (1 - (p / f)) * 100;
                    ...
                    if (allWarnings.length > 0) {
                        ...
                        warnBody.innerHTML = allWarnings.map(w => {
    ...
    if (reportBtn) {
        reportBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                a.href = url;
                a.download = 'PACE_Calculation_Report.docx';
...
window.drawPFDSVG = function(ro_results, p1Arr, nEl) {
    ...
    function txt(x,y,t,anchor='middle',fill='#e2e8f0',fs=9.5,fw='normal') {
    ...
    function feedSym(cx, cy, flow) {
      const flowTxt = flow > 0 ? txt(cx, cy-32, `${flow.toFixed(1)} m³/h`, 'middle', '#3b82f6', 8.5) : '';
      return `<g>
        <path d="M${cx},${cy-20} L${cx-16},${cy+14} L${cx+16},${cy+14}Z" fill="#1e3a8a" stroke="#3b82f6" stroke-width="2"/>
        ${txt(cx, cy+28, 'Feed Water', 'middle', '#f8fafc', 9, 'bold')}
        ${flowTxt}
      </g>`;
    }
    ...
    function filterSym(cx, cy, lbl) {
      ...
      [0.25, 0.50, 0.75].forEach(f => {
        ls += `<line x1="${cx-w/2+4}" y1="${cy-h/2+h*f}" x2="${cx+w/2-4}" y2="${cy-h/2+h*f}" stroke="#94a3b8" stroke-width="1.6"/>`;
      });
      return `<g>
        ${rect(cx-w/2, cy-h/2, w, h, 3, '#1e293b', '#94a3b8')}
        <ellipse cx="${cx}" cy="${cy-h/2}" rx="${w/2}" ry="4.5" fill="#0f172a" stroke="#94a3b8" stroke-width="1.5"/>
        ${ls}
        ${txt(cx, cy+h/2+16, lbl, 'middle', '#cbd5e1', 8.5)}
      </g>`;
    }
    
    function pumpSym(cx, cy, lbl, tag, press) {
      const r = N.PR;
      const pressTxt = press > 0 ? txt(cx, cy-r-10, `${press.toFixed(1)} bar`, 'middle', '#3b82f6', 8.5, 'bold') : '';
      return `<g>
        <circle cx="${cx}" cy="${cy}" r="${r}" fill="#1e293b" stroke="#3b82f6" stroke-width="2.5"/>
        <polygon points="${cx-r*0.55},${cy-r*0.55} ${cx-r*0.55},${cy+r*0.55} ${cx+r*0.75},${cy}" fill="#3b82f6"/>
        ${txt(cx, cy+r+16, lbl, 'middle', '#f8fafc', 9, 'bold')}
        ${txt(cx, cy+r+28, tag, 'middle', '#94a3b8', 8)}
        ${pressTxt}
      </g>`;
    }
    ...
    function boosterSym(cx, cy, boost_dp) {
        const r = 18;
        return `<g>
            <circle cx="${cx}" cy="${cy}" r="${r}" fill="#1e293b" stroke="#f59e0b" stroke-width="2.5"/>
    ...
    function vesselSym(x, y, ne, si, vi, tooltipText='') {
      ...
      for (let i = 0; i < ne - 1; i++) {
        divs += `<line x1="${x+9+ew*(i+1)}" y1="${y+4}" x2="${x+9+ew*(i+1)}" y2="${y+N.VH-4}" stroke="#94a3b8" stroke-width="1" stroke-dasharray="2,2"/>`;
      }
      return `<g style="cursor: pointer;">
        <title>${tooltipText}</title>
        ${rect(x, y, N.VW, N.VH, N.VH/2, '#1e293b', '#475569')}
        ${divs}
        ...
        ${txt(x+N.VW/2, y+N.VH/2+4, `Stage ${si+1} - ${vi+1}`, 'middle', '#cbd5e1', 8.5)}
      </g>`;
    }
    
    function tankSym(cx, cy, lbl, fill='#064e3b', stroke='#10b981', textFill='#34d399') {
      return `<g>
    ...
    const rEdge = Math.max(1000, p1Rx + 200);
    const bEdge = permY + 110;

    let s = '';
    
    // Background
    s += `<rect width="100%" height="100%" fill="#0f172a"/>`;

    // Header Area
    s += `<rect x="0" y="0" width="${rEdge}" height="60" fill="#1e293b"/>`;
    s += `<line x1="0" y1="60" x2="${rEdge}" y2="60" stroke="#334155" stroke-width="1.5"/>`;
    s += txt(20, 26, 'System Process Flow Diagram', 'start', '#f8fafc', 14, 'bold');
    s += txt(20, 46, `Single-Pass RO Array | ${p1Arr.join(':')} configuration | ${nEl} Elements per Vessel`, 'start', '#94a3b8', 10);

    // Pre-treatment lines
    s += line(feedX+16, p1CY, acfX-N.FW/2, p1CY, '#3b82f6', 2.5);
    s += line(acfX+N.FW/2, p1CY, mcfX-N.FW/2, p1CY, '#3b82f6', 2.5);
    s += line(mcfX+N.FW/2, p1CY, hpX-N.PR, p1CY, '#3b82f6', 2.5);
    s += line(hpX+N.PR, p1CY, p1[0].fhX, p1CY, '#3b82f6', 2.5);
    ...
    p1.forEach((st, si) => {
        ...
        s += stageBadge(cx, topY, 'Stage ' + (si+1), '#334155', '#f8fafc', stageData);

        // Vertical Manifolds
        s += `<line x1="${fhX+N.HW/2}" y1="${topY-4}" x2="${fhX+N.HW/2}" y2="${topY+sH+4}" stroke="#3b82f6" stroke-width="6" stroke-linecap="round"/>`; // Feed manifold
        ...
        if (next) {
            ...
            if (bst) {
                ...
                s += line(midX+18, p1CY, toX, p1CY, '#3b82f6', 2.5);   // Blue going out (boosted)
    ...
    // Draw proper arrow pointing to the Permeate Tank (left boundary is at p1PermRx+90-24 = p1PermRx+66)
    s += `<polygon points="${p1PermRx+56},${permY-6} ${p1PermRx+56},${permY+6} ${p1PermRx+66},${permY}" fill="#10b981"/>`;
    
    // Total Permeate Text (placed below the horizontal line to avoid overlapping with vertical dashed lines)
    ...
    s += tankSym(p1PermRx+90, permY, 'Permeate Tank');

    // Final Concentrate
    const cX = p1Rx;
    s += line(cX, p1CY, cX+96, p1CY, '#ef4444', 3.5);
    // Draw proper arrow pointing to the Reject Tank (left boundary is at cX+130-24 = cX+106)
    ...
    s += txt(rejectTextX, p1CY+16, 'Reject Stream', 'middle', '#ef4444', 9.5, 'bold');
    if (netRejectFlow > 0) {
    ...
    s += tankSym(cX+130, p1CY, 'Reject Tank', '#7f1d1d', '#ef4444', '#fca5a5');
    ...
    if (recycleEnabled) {
        ...
        s += txt(midX, 108, flowText, 'middle', '#cbd5e1', 8.5, 'bold');
        ...
        if (blendFlow > 0) {
            ...
            s += txt((hpX + N.PR + p1[0].fhX) / 2, p1CY + 14, 'Blended Feed', 'middle', '#cbd5e1', 8, 'italic');
        }
    }
